...
RegisterServerEvent('grw-roulette:removemoney')
AddEventHandler('grw-roulette:removemoney', function(amount)
	local amount = amount
...
RegisterServerEvent('grw-roulette:givemoney')
AddEventHandler('grw-roulette:givemoney', function(action, amount)
	local aciton = aciton
	local amount = amount
	local _source = source
	local xPlayer = QBCore.Functions.GetPlayer(_source)
...
QBCore.Functions.CreateCallback('grw-roulette:check_money', function(source, cb)
	local _source = source
	local xPlayer = QBCore.Functions.GetPlayer(_source)
	local item = xPlayer.Functions.GetItemByName('casinochips')
	if item ~= nil then
		cb(item.amount)
	else
		cb(0)
