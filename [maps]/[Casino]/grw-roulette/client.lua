...
RegisterNetEvent('grw-roulette:start')
AddEventHandler('grw-roulette:start', function()
	-- QBCore.Functions.TriggerCallback('qb-casinojob:check_gold_status', function(result)
		-- QBCore.Functions.TriggerCallback('qb-casinojob:check_diamond_status', function(result1)
			-- if result or result1 then
				QBCore.Functions.TriggerCallback('grw-roulette:check_money', function(quantity)
...
RegisterNUICallback('betup', function(data, cb)
	cb('ok')
	TriggerServerEvent('InteractSound_SV:PlayOnSource', 'changebet', 1.0)
end)

RegisterNUICallback('roll', function(data, cb)
	cb('ok')
	TriggerEvent('grw-roulette:start_game', data.kolor, data.kwota)
end)

RegisterNetEvent('grw-roulette:start_game')
AddEventHandler('grw-roulette:start_game', function(action, amount)
	local amount = amount
	if amount <= 1000000 then
		
	if game_during == false then
		TriggerServerEvent('grw-roulette:removemoney', amount)
		local kolorBetu = action
		QBCore.Functions.Notify("You have bet "..amount.." chips on "..kolorBetu..". The wheel is spinning...")
		game_during = true
		...
		if action == 'black' then
			if has_value(black, randomNumber) then
				...
				TriggerServerEvent('grw-roulette:givemoney', action, amount)
			else
