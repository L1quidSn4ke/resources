...
QBCore.Commands.Add("startairdrop", "Drop an airdrop", {}, true, function(source)
    SpawnAirdrop()
end, 'admin')
...
QBCore.Functions.CreateUseableItem(Config.Item1, function(source, item)
    ...
    TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items[Config.Item1], 'remove', 1)
    local randomLocation = math.random(1, #Config.Locations)
...
QBCore.Functions.CreateUseableItem(Config.Item2, function(source, item)
    ...
    TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items[Config.Item2], 'remove', 1)
    local randomLocation = math.random(1, #Config.Locations)
    TriggerClientEvent('mh-airdrops:client:deleteObj', src)
    TriggerClientEvent('mh-airdrops:client:airdrop', src, randomLocation)
    TriggerClientEvent('mh-airdrops:client:notify', src, Lang:t('notify.airdrop_landed'), "success", 5000)
...
RegisterNetEvent('mh-airdrops:server:getloot', function(ObjNetId)
    ...
    if whatBooty == 1 then
        ...
        TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items[item.name], 'add', random)
    ...
    elseif whatBooty == 3 then
        ...
        TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items[ammo.amount], 'add', ammo.amount)
    ...
    elseif whatBooty == 4 then
        ...
        TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items[weapon.model], 'add', 1)
    end
    ...
    if Config.UseJackpotItem then
        ...
        if chance < 7 then
			...
			local info = {}
			info.worth = Config.JackpotAmount
            Player.Functions.AddItem('markedbills', bags, false, info)
			TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], "add", bags)
            --[[Player.Functions.AddItem(Config.JackpotItem, Config.JackpotAmount, true)
            TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items[Config.JackpotItem], 'add', Config.JackpotAmount)]]
        end
    end
end)
