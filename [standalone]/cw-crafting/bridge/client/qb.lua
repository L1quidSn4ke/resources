...
function getCraftingSkill(skill)
    if Config.UseCWRepForCraftingSkill then
        ...
        local PlayerData = QBCore.Functions.GetPlayerData()
        return PlayerData.metadata.rep['crafting'] or 0
    end
end
...
function getCraftingLevel(skill)
    if Config.UseCWRepForCraftingSkill then
        ...
        local PlayerData = QBCore.Functions.GetPlayerData()
        if not PlayerData or not PlayerData.metadata.rep['crafting'] then print('Could not find player data') return 0 end
        if not PlayerData.metadata.rep['crafting'] then return 0 end

        return math.ceil(PlayerData.metadata.rep['crafting'] / 100) or 0
    end
end
