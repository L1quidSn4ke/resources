...
local function giveBlueprintItemByType(source, bpType, amount)
    local src = source
    local blueprintType

    if type(bpType) == 'string' then
        blueprintType = bpType
    elseif type(bpType) == 'table' then
        blueprintType = bpType[math.random(1, #bpType)]
    end

    if not blueprintsByType[blueprintType] then
        print("^1ERROR: Blueprint type '" .. tostring(blueprintType) .. "' not found^7")
        return false
    end

    local blueprintsWeighted = {}
    local totalWeight = 0

    for bp, data in pairs(blueprintsByType[blueprintType]) do
        local rarityFactor = 6 - (data.rarity or 1)
        local chanceFactor = data.chance or 0.5

        local weight = rarityFactor * chanceFactor * 100

        table.insert(blueprintsWeighted, {
            blueprint = bp,
            weight = weight
        })
    ...
    if #blueprintsWeighted == 0 then
        print("^1ERROR: No blueprints found for type '" .. blueprintType .. "'^7")
        return false
    end

    local randomValue = math.random() * totalWeight
    local currentWeight = 0
    local blueprint, label

    for _, entry in ipairs(blueprintsWeighted) do
    ...
    if not blueprint then
        blueprint = blueprintsWeighted[#blueprintsWeighted].blueprint
    end

    label = Config.Blueprints[blueprint].label or blueprint
    ...
    if amount and amount > 1 then
        giveBlueprintItemByType(src, bpType, amount - 1)
    end

    return true
end
exports('giveBlueprintItemByType', giveBlueprintItemByType)
...
RegisterNetEvent('cw-crafting:server:addBlueprintFromLearning', function(blueprint)
    local src = source
    local value = blueprint.bpName
    if useDebug then
        print('used blueprint', json.encode(blueprint, {indent = true}))
    end
    local bpItems = exports['ps-inventory']:GetItemsByName(src, Config.ItemName)
    for index, itemData in pairs(bpItems) do
        if itemData.info.value == value then
            handleAddBlueprintFromItem(src, blueprint.bpName, itemData.slot)
            return
        end
    end
end)
...
registerCommand('addblueprint', 'Give blueprint knowledge to player. (Admin Only)', {{name = 'player id', help = 'the id of the player'}, {name = 'blueprint', help = 'name of blueprint'}}, true, function(source, args)
    local citizenId = getCitizenId(tonumber(args[1]))
...
registerCommand('giveblueprintbytype', 'Add blueprint item to player. (Admin Only)', {{name = 'player id', help = 'the id of the player'}, {name = 'blueprinttype', help = 'type of blueprint'}}, true, function(source, args)
    giveBlueprintItemByType(tonumber(args[1]), args[2])
end, 'admin')
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    for bp, data in pairs(Config.Blueprints) do
        if data.type then
            if not blueprintsByType[data.type] then
                blueprintsByType[data.type] = {}
            end
            blueprintsByType[data.type][bp] = data
        end
    end

    if Config.Debug then
        for typeName, typeTable in pairs(blueprintsByType) do
            local count = 0
            for _ in pairs(typeTable) do count = count + 1 end
            print(string.format("Loaded %d blueprints of type '%s'", count, typeName))
        end
    end
end)